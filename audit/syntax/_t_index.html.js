...
const state={ name:'', phone:'', contactConsent:false, a:{}, lead_id:null, ref:genRef() };
...
function submitData(){
  ...
  if(!state.contactConsent)return;
  persistLead();
...
async function persistLead(){
  try{
    ...
    const r=await fetch(WORKER+'/api/leads',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({tenant_id:TENANT_ID,name:state.name,phone:state.phone,treatment:CFG.treatment,motivo,plazo,objecion,quiz_score:score,temperature,source:'embudo_'+TREATMENT_ID,ref:state.ref,contact_consent:state.contactConsent,contact_consent_proof:'embudo:'+TREATMENT_ID+':service_contact_v1'})});
