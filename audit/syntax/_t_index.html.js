...
const state={ name:'', phone:'', contactConsent:false, turnstileToken:'', a:{}, lead_id:null, lead_token:null, ref:genRef() };
...
(async function resumeIfMagic(){
  ...
  try{
    ...
    if(!d.ok) return;
    state.lead_id=lead; state.lead_token=d.lead_token||k; if(d.lead){ state.name=d.lead.name||''; state.phone=d.lead.phone||''; }
...
function renderQ(){
  ...
  if(Q.form){
    body.innerHTML=`
      ...
      <div id="fTurnstile" style="min-height:65px;display:flex;justify-content:center;margin:.35rem 0"></div>
      <p id="fSecurityMsg" style="min-height:1rem;text-align:center;color:#9b332b;font-size:.72rem"></p>
    ...
    function chk(){ s.classList.toggle('ready', n.value.trim().length>1 && p.value.replace(/\D/g,'').length>=9 && c.checked && !!state.turnstileToken); }
    n.oninput=chk; p.oninput=chk; c.onchange=chk;
    let tries=0; const mountTurnstile=()=>{ if(window.turnstile){ window._auraLeadTurnstile=turnstile.render('#fTurnstile',{sitekey:'0x4AAAAAAEYnYxmFSrnwB2Bm',action:'lead',theme:'light',size:'flexible',callback:(token)=>{state.turnstileToken=token||'';chk();},'expired-callback':()=>{state.turnstileToken='';chk();},'error-callback':()=>{state.turnstileToken='';chk();}}); } else if(++tries<40){setTimeout(mountTurnstile,150);} }; setTimeout(mountTurnstile,0);
    return;
  }
  body.innerHTML=`
    <div class="qq">${Q.q}</div>
...
async function submitData(){
  ...
  state.contactConsent=!!document.getElementById('fContactConsent').checked;
  const securityMsg=document.getElementById('fSecurityMsg');
  if(!state.contactConsent||!state.turnstileToken){if(securityMsg)securityMsg.textContent='Completa la verificación de seguridad.';return;}
  const submit=document.getElementById('fSubmit'); submit.disabled=true;
  if(!(await persistLead())){if(securityMsg)securityMsg.textContent='No hemos podido verificarte. Inténtalo de nuevo.';state.turnstileToken='';if(window.turnstile&&window._auraLeadTurnstile!==undefined)turnstile.reset(window._auraLeadTurnstile);submit.disabled=false;return;}
...
async function persistLead(){
  try{
    ...
    const leadTenant=window.DEMO_SALES?'focus-ventas':TENANT_ID;
    const r=await fetch(WORKER+'/api/leads',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({tenant_id:leadTenant,name:state.name,phone:state.phone,treatment:CFG.treatment,motivo,plazo,objecion,quiz_score:score,temperature,source:'embudo_'+TREATMENT_ID,ref:state.ref,contact_consent:state.contactConsent,contact_consent_proof:'embudo:'+TREATMENT_ID+':service_contact_v1',turnstile_token:state.turnstileToken})});
    if(r.ok){ const d=await r.json(); state.lead_id=d.lead_id||d.id||null; state.lead_token=d.lead_token||null; return !!(state.lead_id&&state.lead_token); }
  }catch(e){}
  return false;
}
async function updateLeadAnswers(){
  ...
  try{
    ...
    await fetch(WORKER+'/api/lead-quiz-update',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({lead_id:state.lead_id,lead_token:state.lead_token,motivo,plazo,objecion,quiz_score:score,temperature})});
  }catch(e){}
}
...
async function sendMsg(){
  ...
  if(!state._chatted){ state._chatted=true; try{ if(state.lead_id) fetch(WORKER+'/api/lead-chatted',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({lead_id:state.lead_id,lead_token:state.lead_token})}); }catch(e){} }
  await wait(350); typing();
  try{
    const r=await fetch(WORKER+'/chat',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({messages:SETTER_HISTORY,tenant_id:TENANT_ID,lead_id:state.lead_id,lead_token:state.lead_token,context:{name:state.name,treatment:CFG.treatment,goal:state.a?.motivo||'',plazo:state.a?.plazo||'',objecion:state.a?.objecion||'',lead_id:state.lead_id,conversation_id:SETTER_CONVERSATION_ID,mode:window.DEMO_SALES?'sales_demo':'patient'},brain_state:state.setter_brain||{}})});
...
async function book(el,slot,iso){
  ...
  if(window.DEMO_SALES){
    ...
    try{ await fetch(WORKER+'/api/appointments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tenant_id:'focus-ventas',lead_id:state.lead_id,lead_token:state.lead_token,treatment:'Consultoría AURA · '+(window.DEMO_CLINIC_NAME||''),date_iso:iso||undefined,duration_min:60,slot,ref:state.ref})}); }catch(e){}
    return;
  }
  ...
  try{ await fetch(WORKER+'/api/appointments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tenant_id:TENANT_ID,lead_id:state.lead_id,lead_token:state.lead_token,treatment:(esRecall?'Revisión / retoque':CFG.treatment),date_iso:iso||undefined,slot,ref:state.ref})}); }catch(e){}
}
function markWa(){
  try{ fetch(WORKER+'/api/lead-event',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tenant_id:TENANT_ID,lead_id:state.lead_id,lead_token:state.lead_token,ref:state.ref,event:'whatsapp_opened'})}); }catch(e){}
