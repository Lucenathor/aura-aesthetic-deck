...
let qi=0;
function startQuiz(){ applyQuizContent(); qi=0; renderQ(); }
...
async function sendMsg(){
  ...
  try{
    const r=await fetch(WORKER+'/chat',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({messages:[{role:'user',content:v}],tenant_id:TENANT_ID,lead:{name:state.name,treatment:CFG.treatment,answers:state.a}})});
