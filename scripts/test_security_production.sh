...
check_code(){
  ...
  local got
  got=$(curl -sS --retry 2 --retry-all-errors --retry-delay 2 --connect-timeout 10 -o /tmp/aura_security_response -w '%{http_code}' --max-time 35 "$@" || true)
...
check_code "Webhook 360dialog de mensajes exige firma" 401 -X POST "$API/api/wa-webhook-360" -H 'Content-Type: application/json' --data '{}'
check_code "Webhook 360dialog Partner exige firma" 401 -X POST "$API/api/360-partner-webhook" -H 'Content-Type: application/json' --data '{}'
check_code "Métricas exigen sesión" 403 "$API/api/advanced-metrics?tenant=aura-demo"

evil_headers=$(curl -sS --retry 2 --retry-all-errors --retry-delay 2 --connect-timeout 10 -D - -o /dev/null -X OPTIONS "$API/api/advanced-metrics?tenant=aura-demo" -H 'Origin: https://evil.example' -H 'Access-Control-Request-Method: GET' | tr -d '\r')
...
good_headers=$(curl -sS --retry 2 --retry-all-errors --retry-delay 2 --connect-timeout 10 -D - -o /dev/null -X OPTIONS "$API/api/advanced-metrics?tenant=aura-demo" -H 'Origin: https://auracrm.co' -H 'Access-Control-Request-Method: GET' | tr -d '\r')
...
worker_headers=$(curl -sS --retry 2 --retry-all-errors --retry-delay 2 --connect-timeout 10 -D - -o /dev/null "$API/api/auth/me" | tr -d '\r')
...
if [ "${AURA_SKIP_PAGES:-0}" != "1" ]; then
  pages_headers=$(curl -sS --retry 2 --retry-all-errors --retry-delay 2 --connect-timeout 10 -D - -o /dev/null "$SITE/login" | tr -d '\r')
  ...
  curl -sS --max-time 45 "$SITE/login" -o /tmp/aura_security_login.html
  ...
  curl -sS --max-time 60 "$SITE/c/aura-demo" -o /tmp/aura_security_funnel.html
